...
func (s IdentityStore) CreateChallenge(ctx context.Context, c identity.Challenge) error {
	...
	// Lock the parent even when no challenge exists yet. All challenge operations use this lock order.
	var id string
	if err = tx.QueryRow(ctx, `SELECT id FROM users WHERE id=$1 AND status='active' FOR UPDATE`, c.UserID).Scan(&id); err != nil {
		return err
	}
...
func consume(ctx context.Context, tx pgx.Tx, p identity.Purpose, d [32]byte, now time.Time) (string, error) {
	...
	err := tx.QueryRow(ctx, `SELECT u.id FROM users u JOIN identity_challenges c ON c.user_id=u.id WHERE c.purpose=$1 AND c.secret_hash=$2 AND u.status='active' FOR UPDATE OF u`, string(p), d[:]).Scan(&userID)
	if errors.Is(err, pgx.ErrNoRows) {
...
func (s IdentityStore) ResetPassword(ctx context.Context, d [32]byte, passwordHash string, now time.Time) (*identity.User, error) {
	tx, err := s.DB.BeginTx(ctx, pgx.TxOptions{})
	if err != nil {
		return nil, err
	}
	...
	if err = tx.QueryRow(ctx, `SELECT id,email FROM users WHERE id=$1`, userID).Scan(&user.ID, &user.Email); err != nil {
		return nil, err
	}
